...
AppEntry::AppEntry(QObject* entry, unsigned int frequency, QObject* parent)
    : QObject(parent), m_entry(entry), m_frequency(frequency) {
        ...
        for (const auto& prop :
             {"name", "comment", "execString", "startupClass", "genericName",
              "categories", "keywords"}) {
                const auto metaProp = mo->property(mo->indexOfProperty(prop));
                const auto thisMetaProp =
                    tmo->property(tmo->indexOfProperty(prop));
...
AppDb::AppDb(QObject* parent)
    : QObject(parent), m_timer(new QTimer(this)),
      m_uuid(QUuid::createUuid().toString()) {
        ...
        QSqlQuery query(db);
        query.exec("CREATE TABLE IF NOT EXISTS frequencies (id TEXT PRIMARY "
...
void AppDb::incrementFrequency(const QString& id) {
        ...
        if (app) {
                ...
                qWarning()
                    << "AppDb::incrementFrequency: could not find app with id"
                    << id;
        }
}

QQmlListProperty<AppEntry> AppDb::getFilteredApps(const QString& filter,
                                                  const qint32 maxResults) {
        m_filteredApps.clear();
        QList<std::tuple<AppEntry*, qint32>> unfilteredApps;
        Bitap bitap = Bitap();
        bitap.setK(5);
        qint32 nrErrors = 0;
        for (auto* app : m_apps.values()) {
                nrErrors = bitap.search(app->name().toUpper().toStdString(),
                                        filter.toUpper().toStdString());
                if (nrErrors >= 0) {
                        unfilteredApps.append(std::make_tuple(app, nrErrors));
                }
        }
        std::sort(unfilteredApps.begin(), unfilteredApps.end(),
                  [](const std::tuple<AppEntry*, qint32>& a,
                     const std::tuple<AppEntry*, qint32>& b) {
                          if (std::get<1>(a) == std::get<1>(b)) {
                                  return std::get<0>(a)->frequency() >
                                         std::get<0>(b)->frequency();
                          }
                          return std::get<1>(a) < std::get<1>(b);
                  });
        for (auto app : unfilteredApps) {
                m_filteredApps.push_back(std::get<0>(app));
        }

        // m_filteredApps = sortApps(m_filteredApps);
        if (m_filteredApps.count() > maxResults)
                m_filteredApps = m_filteredApps.sliced(0, maxResults);
...
QList<AppEntry*>& AppDb::sortApps(QList<AppEntry*>& unsortedApps) const {
        std::sort(unsortedApps.begin(), unsortedApps.end(),
                  [](AppEntry* a, AppEntry* b) {
                          if (a->frequency() != b->frequency()) {
                                  return a->frequency() > b->frequency();
                          }
...
void AppDb::updateApps() {
        ...
        for (const auto& entry : std::as_const(m_entries)) {
                ...
                if (!m_apps.contains(id)) {
                        dirty = true;
                        auto* const newEntry =
                            new AppEntry(entry, getFrequency(id), this);
